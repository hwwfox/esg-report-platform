...
export function WorkbenchPage() {
  ...
  return (
    <main style={{ padding: 24 }}>
      <Card extra={<Space><Link to="/enterprise-projects">企业与项目</Link><Link to="/peer-analysis">GICS与同行池</Link><Link to="/peer-reports">同行报告</Link><Link to="/recommendations">推荐标准与议题</Link><Link to="/esg-data">ESG数据表</Link>{canReadStandardLibrary && <Link to="/standard-library">标准库</Link>}<Button onClick={handleLogout}>退出登录</Button></Space>}>
