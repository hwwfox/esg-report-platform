...
): Promise<ListResponse<Standard>> {
  return parseResponse<ListResponse<Standard>>(
    await fetch(`${API_BASE}/standards${queryString(params)}`, {
...
export async function listTopics(
  ...
  return parseResponse<ListResponse<Topic>>(
    await fetch(`${API_BASE}/topics${queryString(params)}`, {
      headers: authHeaders(token),
    }),
  );
}
...
export async function listMetrics(
  ...
  return parseResponse<ListResponse<Metric>>(
    await fetch(`${API_BASE}/metrics${queryString(params)}`, {
      headers: authHeaders(token),
    }),
  );
}

export async function getRecommendedMetrics(
  token: string,
  topicCode: string,
): Promise<RecommendedMetricsResponse> {
  return parseResponse<RecommendedMetricsResponse>(
    await fetch(
